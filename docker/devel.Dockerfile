...
ARG CUDA_MAJOR=12
ARG CUDA_MINOR=4
ARG CUDA_ADDITIONAL="cudnn9-devel"
ARG DOCKER_IMG_REPO="pytorch/pytroch"

ARG BASE_IMAGE=${DOCKER_IMG_REPO}:${PYTORCH_MAJOR}.${PYTORCH_MINOR}.${PYTROCH_PATCH}-${CUDA_MAJOR}.${CUDA_MINOR}-${CUDA_ADDITIONAL}
FROM ${BASE_IMAGE}
