...
import sys
# sys.path.append("/home/trushant/SeeedHackathon/lepanipuri/Isaac-GR00T/gr00t/eval")
sys.path.append("/home/Isaac-GR00T/gr00t/eval")
from service import ExternalRobotInferenceClient
sys.path.append("/home/Isaac-GR00T/examples/SO-100")
from tmp_ensembling import TimeEnsembler
...
class Gr00tRobotInferenceClient:
    ...
    def get_action(self, observation_dict, lang: str):
        ...
        obs_dict["state.left_arm"] = state[:5].astype(np.float64)
        obs_dict["state.left_gripper"] = state[5:6].astype(np.float64)
        obs_dict["state.right_arm"] = state[6:11].astype(np.float64)
        obs_dict["state.right_gripper"] = state[11:12].astype(np.float64)
...
def eval(cfg: EvalConfig):
    ...
    left_temp_ensembling = TimeEnsembler(balancing_factor=0.1,
                                    horizon=16,
                                    n_joints=6)
    right_temp_ensembling = TimeEnsembler(balancing_factor=-1.0,
                                    horizon=16,
                                    n_joints=6)
    ...
    while True:
        ...
        # observation_dict["top"] = np.zeros((480, 640, 3), dtype=np.uint8)
        ...
        left_action_chunk_list = []
        for idx in range(len(action_chunk)):
            action = []
            for key in action_chunk[idx].keys():
                if not key.__contains__("left"):
                    continue
                action.append(action_chunk[idx][key])
            left_action_chunk_list.append(action)
        right_action_chunk_list = []
        for idx in range(len(action_chunk)):
            action = []
            for key in action_chunk[idx].keys():
                if not key.__contains__("right"):
                    continue
                action.append(action_chunk[idx][key])
            right_action_chunk_list.append(action)
        
        left_action_chunk_list = np.array(left_action_chunk_list)
        right_action_chunk_list = np.array(right_action_chunk_list)

        left_action_op = left_temp_ensembling.ensemble(left_action_chunk_list)
        right_action_op = right_temp_ensembling.ensemble(right_action_chunk_list)
        single_action_chunk = {}
        idx = 0
        for key in action_chunk[0].keys():
            if key.__contains__("left"):
                single_action_chunk[key] = left_action_op[idx]
            elif key.__contains__("right"):
                single_action_chunk[key] = right_action_op[idx-6]
            idx +=1 
        robot.send_action(single_action_chunk)
        time.sleep(0.02)
