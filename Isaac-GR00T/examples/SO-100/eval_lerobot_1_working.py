...
def eval(cfg: EvalConfig):
    ...
    temp_ensembling = TimeEnsembler(balancing_factor=0.01,
                                    horizon=16,
                                    n_joints=12)
    ...
    while True:
        ...
        for idx in range(len(action_chunk)):
            ...
            action_chunk_list.append(action)
        action_chunk_list = np.array(action_chunk_list)
        action_op = temp_ensembling.ensemble(action_chunk_list)
        single_action_chunk = {}
        idx = 0
        for key in action_chunk[0].keys():
            single_action_chunk[key] = action_op[idx]
            idx +=1 
